!***************************************************************************
!     c	COMPACT FINITE DIFFERENCE SUBROUTINES
!c	O.F, nov 2002
!c
!c	updated dic 2002
...
!c*      p:     flag:  0 -> uniform mesh otherwise non-uniform    *
!c*		               p=1 -> tanh			                            *
!c*	                 p=2 -> sin			                              *
!c*      c:      stretching parameter                             *
...
!    if (myid==0) write(*,*) 'uniform mesh'
      endif
      if (p.eq.1) then         !!! malla tangente hiperbolica
!         if (myid==0)  write(*,*) 'generating tanh mesh'
         do j=1,n
...
!         if (myid==0) write(*,*) 'generating sen mesh'
         do j=1,n
            h(j) = sin(c*(2d0*(j-1)/(n-1)-1d0)*pi/2d0)/sin(c*pi/2d0)
            h2(j)= sin(c*(2d0*float(j-1)/float(n-1)-1d0)*pi2/2d0)/sin(c*pi2/2d0)
            g(j) = 2d0*sin(c*pi/2d0)/(cos(c*(2d0*(j-1)/(n-1)-1d0)*pi/2d0)*c*pi)
...
!c*	prederiv1:							                                                   !
!c*      							                                                      !
!c*	Generates coefficients alpha,beta,a0,a1 y a2 for                         !
!c*        compact finite difference scheme with uniform mesh                 !
!c*								              !
!c*	beta*(du(j+2)+du(j-2))+alpha*(du(j+1)+du(j-1))+du(j) =                !
!c*	 = a1*(u(j+1)-u(j-1)) + a2(u(j+2)-u(j-2))	                      !
!c*				                                              !
!c*		At the boundaries of the domain:                              !
!c*		alpha*du(2) + du(1) =                                         !
...
!c*	prederiv2:
!c*
!c*	Generates coefficients alpha,beta,a0,a1 y a2
!c*	for compact finite difference scheme
!c*
...
!c				  !!!  calculo el termino independiente
      ...
      wk1(1)=dt12(3,1)*uwk(1,1)+dt12(4,1)*uwk(2,1)+dt12(5,1)*uwk(3,1)
      wk1(2)=dt12(2,2)*uwk(1,1)+dt12(3,2)*uwk(2,1)+dt12(4,2)*uwk(3,1)+dt12(5,2)*uwk(4,1)
      do j=3,my-2
         ...
         enddo
      enddo
      wk1(my-1)=dt12(1,my-1)*uwk(my-3,1)+dt12(2,my-1)*uwk(my-2,1) + dt12(3,my-1)*uwk(my-1,1)+dt12(4,my-1)*uwk(my,1)
      wk1(my)  =dt12(1,my)*uwk(my-2,1)+dt12(2,my)*uwk(my-1,1) + dt12(3,my)*uwk(my,1)

      wk2(1)=dt12(3,1)*uwk(1,2)+dt12(4,1)*uwk(2,2)+dt12(5,1)*uwk(3,2)
      wk2(2)=dt12(2,2)*uwk(1,2)+dt12(3,2)*uwk(2,2)+dt12(4,2)*uwk(3,2)+dt12(5,2)*uwk(4,2)
      do j=3,my-2
         ...
         enddo
      enddo
      wk2(my-1)=dt12(1,my-1)*uwk(my-3,2)+dt12(2,my-1)*uwk(my-2,2)+ dt12(3,my-1)*uwk(my-1,2)+dt12(4,my-1)*uwk(my,2)
      wk2(my)  =dt12(1,my)*uwk(my-2,2)+dt12(2,my)*uwk(my-1,2)+dt12(3,my)*uwk(my,2)
...
!c-------PREPARO EL TERMINO INDEPENDIENTE [d2]*{u} !!!d2 casi-pentadiagonal!!!
      ...
      fwk(2) =dt12(2,2)*uwk(1)+dt12(3,2)*uwk(2)+dt12(4,2)*uwk(3)+dt12(5,2)*uwk(4)
      do j=3,my-2
      ...
      enddo
      enddo
      fwk(my-1)=dt12(1,my-1)*uwk(my-3)+dt12(2,my-1)*uwk(my-2)+dt12(3,my-1)*uwk(my-1)+dt12(4,my-1)*uwk(my)
      fwk(my)=  dt12(1,my)*uwk(my-2)  +dt12(2,my)*uwk(my-1)+dt12(3,my)*uwk(my)
